...
# ── Generatore Factsheet ─────────────────────────────────────────────────────
def genera_factsheet(titolo, fonte, scadenza, link, tipo_beneficiario, qualita, motivazione_ai):
    """Legge la pagina del bando e genera un factsheet HTML con Claude Sonnet."""
...
Se un dato non è disponibile nel testo, usa "N/D". Per mix_fp_pct e mix_fin_pct usa numeri interi (sommano a 100). Se non c'è un mix chiaro usa 100 e 0."""
    ...
    if dati.get("note_aggiuntive") and dati["note_aggiuntive"].strip() and dati["note_aggiuntive"] != "N/D":
        note_html = f'<div class="intro-box" style="margin-top:0;margin-bottom:18px;border-left-color:#0F6E56"><strong>Note:</strong> {dati["note_aggiuntive"]}</div>'
...
<style>
  *{{margin:0;padding:0;box-sizing:border-box}}
  ...
  .pf-center{{font-size:9px;font-style:italic;color:var(--gt)}}
  .pf-right{{font-size:9.5px;color:var(--vs)}}
  .logo-hdr{{display:flex;align-items:center;gap:10px}}
...
<div class="numeri-grid">
  <div class="numero-card">
    <div class="valore">{dati.get("dotazione_totale","N/D")}</div>
    <div class="label">Dotazione Totale Disponibile</div>
  </div>
  <div class="numero-card">
    <div class="valore">{dati.get("agevolazione_principale","N/D")}</div>
    <div class="label">Agevolazione Principale</div>
    <div class="sub-label">{dati.get("agevolazione_nota","")}</div>
  </div>
  <div class="numero-card">
    <div class="valore">{dati.get("investimento_range","N/D")}</div>
    <div class="label">Investimento Ammissibile</div>
    <div class="sub-label">{dati.get("investimento_nota","")}</div>
  </div>
</div>
...
<div class="invest-grid">
  <div class="invest-card">
    <div class="ic-label">Investimento Minimo</div>
    <div class="ic-value">{dati.get("investimento_minimo","N/D")}</div>
    <div class="ic-note">{dati.get("investimento_minimo_nota","")}</div>
  </div>
  <div class="invest-card">
    <div class="ic-label">Investimento Massimo</div>
    <div class="ic-value">{dati.get("investimento_massimo","N/D")}</div>
    <div class="ic-note">{dati.get("investimento_massimo_nota","")}</div>
  </div>
</div>
...
<div class="two-col">
  <div class="col-box">
    <div class="col-box-title">Chi può candidarsi</div>
    <ul>{chi_list}</ul>
  </div>
  <div class="col-box">
    <div class="col-box-title">Tempistiche principali</div>
    {temp_rows}
  </div>
</div>
...
<div class="cta-footer">
  <div>
    ...
    <div class="cta-desc">Verifica di eligibilità · Strutturazione del programma · Predisposizione della domanda · Monitoraggio e rendicontazione</div>
  </div>
  <div class="cta-right">
    <p>info@lumenadvisors.it</p>
    <p>+41 79 601 5800</p>
    <p>www.lumenadvisors.it</p>
  </div>
</div>
...
<div class="page-footer">
  ...
  <div class="pf-center">Documento riservato · Uso esclusivo del destinatario</div>
        ...
        if not link_bando:
            ...
            if genera_btn:
                with st.spinner("⏳ Claude sta leggendo il bando e generando il factsheet..."):
                    try:
                        html_factsheet = genera_factsheet(
                            ...
                            qualita=str(bando.get("Qualità Bando", "N/D")),
                            motivazione_ai=str(bando.get("Motivazione AI", ""))
                        )
                        nome_file = bando["Titolo"].lower().replace(" ", "_").replace("/", "-")[:40]
                        st.success("✅ Factsheet generato!")
                        st.download_button(
                            label="⬇️ Scarica Factsheet HTML",
                            data=html_factsheet.encode("utf-8"),
                            file_name=f"factsheet_{nome_file}_{datetime.now().strftime('%Y%m%d')}.html",
                            mime="text/html",
                            type="primary"
                        )
                        st.caption("Apri il file nel browser e usa Stampa → Salva come PDF per ottenere il PDF.")
                    except Exception as e:
